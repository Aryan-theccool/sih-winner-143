...
/**
 * Prepare vessels for deck.gl TripsLayer: interpolate dark gaps into the
 * path so vessel motion stays continuous, and precompute lookup for the
 * instantaneous position query.
 */
  ...
  return raw.vessels.map((v) => {
    ...
    for (let i = 0; i < v.path.length; i++) {
      ...
      if (i > 0) {
        ...
        const dtp = p[2] - q[2]
        if (dtp > 1800) {
          // dashed ghost: fill interpolated positions every 10 min
          const n = Math.floor(dtp / 600)
          for (let k = 1; k <= n; k++) {
            const f = (600 * k) / dtp
            pts.push([q[0] + (p[0] - q[0]) * f, q[1] + (p[1] - q[1]) * f, q[2] + 600 * k])
          }
        }
      }
      pts.push(p)
    ...
    return {
      ...
      absolute: pts, // [lon,lat,epoch]
      dark_segments: v.dark_segments,
    }
  })
}

/** interpolate vessel position at epoch seconds (null if outside coverage) */
export function posAt(vessel, epoch) {
  const a = vessel.absolute
  if (epoch < a[0][2] - 60 || epoch > a[a.length - 1][2] + 60) return null
  // binary search
